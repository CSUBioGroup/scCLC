...
def run_leiden(latent_vector, resolution=0.6):
    # 构建距离矩阵或相似度矩阵
    # 这里简单地假设使用欧氏距离
    distances = np.linalg.norm(latent_vector[:, None] - latent_vector, axis=-1)

    # 创建igraph图对象
    G = ig.Graph.Weighted_Adjacency(distances.tolist(), mode=ig.ADJ_UPPER)

    # 使用Leiden算法进行聚类
    partition = G.community_leiden(weights='weight', resolution_parameter = resolution)
...
def run_louvain(latent_vector, resolution=0.6):
    # 计算相似度矩阵，这里简单地假设使用欧氏距离
    distances = np.linalg.norm(latent_vector[:, None] - latent_vector, axis=-1)
    # 计算相似度
    similarities = np.exp(-distances)  # 这里简单地假设使用指数函数转换距离为相似度

    # 创建igraph图对象
    G = ig.Graph.Weighted_Adjacency(similarities.tolist(), mode=ig.ADJ_UPPER)
...
def run_louvain_sklearns(latent_vector, resolution=0.6):
    pearson_sim = np.corrcoef(latent_vector)
    # 计算相似度矩阵，这里简单地假设使用欧氏距离
    #distances = np.linalg.norm(latent_vector[:, None] - latent_vector, axis=-1)
    # 计算相似度
    #similarities = np.exp(-distances)  # 这里简单地假设使用指数函数转换距离为相似度
        #print("Louvain Clustering results on high features:")
        
    louvain = Louvain(resolution = resolution)
    labels = louvain.fit_transform(pearson_sim)
    
    return labels

def run_leiden_sklearns(latent_vector, resolution=0.6):
    #pearson_sim = np.corrcoef(data)
    # 计算相似度矩阵，这里简单地假设使用欧氏距离
    distances = np.linalg.norm(latent_vector[:, None] - latent_vector, axis=-1)
    # 计算相似度
    similarities = np.exp(-distances)  # 这里简单地假设使用指数函数转换距离为相似度
    ...
    leiden  = Leiden(resolution)
    labels = leiden.fit_transform(similarities)
